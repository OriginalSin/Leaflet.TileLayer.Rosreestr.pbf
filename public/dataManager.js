(function () {
	...
	*/
	...
	var Features = {
	  getFeature: function getFeature(id, type) {
	    var url = Config.pkkPrefix + 'api/features/' + type + '/' + id + '?date_format=%c&_=' + Date.now();
	    return fetch(url).then(function (res) {
	      return res.json();
	    }).then(function (json) {
	      return {
	  ...
	  getFeatures: function getFeatures(point) {
	    var url = Config.pkkPrefix + 'api/features/?tolerance=16&skip=0&inPoint=true&text=' + point;
	    return fetch(url).then(function (res) {
	      if (res.status === 404) {
	...
	var getFeature = function getFeature(it, out) {
	  Features.getFeature(it.attrs.id, it.type).then(function (json) {
	    if (json) {
	...
	onmessage = function onmessage(evt) {
	  ...
	  switch (cmd) {
	    ...
	    case 'features':
	      Features.getFeatures(point).then(function (json) {
