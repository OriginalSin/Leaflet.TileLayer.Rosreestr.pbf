...
function MSQR(src, options) {
	...
	if (src instanceof ArrayBuffer ) {
		...
		let pixels = new ImageData(options.width, options.height);
...
ctx.putImageData(pixels, 0, 0);
	...
	function img2context(src) {
		let w = src.naturalWidth || src.videoWidth || src.width;
		let h = src.naturalHeight || src.videoHeight || src.height;
		var c = new OffscreenCanvas(h, h);
