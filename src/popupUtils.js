const cadastreLayers = [
	...
	{id: 20, title: 'Лесничество', 		reg: /^\w+$/},
	// /[^\d\:]/g,
	// /\d\d:\d+$/,
	// /\d\d:\d+:\d+$/,
	// /\d\d:\d+:\d+:\d+$/
];
const getCadastreLayer = (str, type) => {
	// str = str.trim();
	for (let i = 0, len = cadastreLayers.length; i < len; i++) {
		let it = cadastreLayers[i];
...
// https://pkk.rosreestr.ru/#/search/55.59958494444303,37.529283264270816/18/@5w3tqxnc7?text=77%3A6%3A9003%3A8026&type=1&nameTab&indexTab&opened=77%3A6%3A9003%3A8026
	...
	return `<div class="cadNav">
				<span class="cadLeft"${nm ? '' : ' style="visibility: hidden;"'}>&lt;</span>
				<span class="cadCount">${title}</span>
				<span class="cadRight"${nm < features.length -1 ? '' : ' style="visibility: hidden;"'}>&gt;</span>
			</div>`;

};
const featureCont = (feature, title) => {
	const attrs = feature.attrs;
	const address = attrs.address || attrs.desc || '';
	const name = attrs.name || attrs.name_zone || '';
	const date = attrs.cad_record_date || attrs.rec_date || '';
	...
	if (attrs.statecd) { trs.push('<tr><td class="first">Статус:</td><td>' + states[attrs.statecd] + '</td></tr>'); }
	if (name) {
...
export default {	
	getContent: function(feature, nm, features) {
		const it = features[nm];
		const id = it.attrs.id;
		...
		return `
			<div class="cadItem">
				...
				<div class="featureCont">
					...
					${featureCont(feature, cLayer.title)}
					</tbody>
					</table>
				</div>
				<div class="operCont">
